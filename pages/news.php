...
$fd = fopen($_SERVER["DOCUMENT_ROOT"]."/pages/year.dat", 'r') or die("Не удалось открыть файл");
$newYearDate;
$anniversaryDate;
$i = 0;
while(!feof($fd) && ($i < 2))
...
if (strtotime($currentDate)==(strtotime($newYearDate))) { 
	...
	fclose($a2);	 
	$currYear = date("Y");
	$f=fopen($_SERVER["DOCUMENT_ROOT"]."/pages/news.htm","w"); // открываем для записи
    ...
    unset($what7);
	$currYear++;
	$fd = fopen($_SERVER["DOCUMENT_ROOT"]."/pages/year.dat", 'w') or die("Не удалось создать файл");
